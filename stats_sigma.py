...
"""
Analyze and visualize sigma experiment results.
...
def load_episode_data(exp_dirs):
    ...
    Each row represents an episode from a specific run, with agent type and sigma inferred from directory name.
    Args:
    ...
    for exp_dir in map(Path, exp_dirs):
        ...
        # Infer sigma and agent type from directory name
        ...
        try:
            ...
            raise ValueError(
                f"Directory naming must follow pattern 'experiment-sigma<val>-<algo>', got: {exp_dir.name}"
            )
...
def plot_learning_curves_from_df(
    ...
    Args:
        ...
        group_vars (tuple): Columns to group by (e.g., agent, sigma).
...
episodes = load_episode_data(['experiment-sigma0.0-dqn', 'experiment-sigma0.0-ppo', 'experiment-sigma0.1-dqn', 'experiment-sigma0.1-ppo','experiment-sigma0.2-dqn', 'experiment-sigma0.2-ppo'])

# Compute success rate per run and the mean of the total return in the final episodes
grouped = (episodes
           .groupby(["agent", "sigma", "run_id"])
           .agg(
...
grouped.to_csv(output_dir / "summary_sigma_experiment.csv", index=False)
