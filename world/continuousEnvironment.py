...
class ContinuousEnvironment:
    ...
    def _move_agent(self, new_pos):
        ...
        for corner in corners:
            ...
            if 0 <= gp[0] < self.grid.shape[0] and 0 <= gp[1] < self.grid.shape[1]:
                if self.grid[gp] == 3:
                    self.grid[gp] = 0
    ...
    @staticmethod
    def evaluate_agent(grid_fp, agent, max_steps, sigma=0.0, agent_start_pos=None, random_seed=0, show_images=False, verbose=True):
        ...
        if verbose:
            ...
            print("cumulative_reward:", total)
            print("total_steps:", len(path))
            print("total_agent_moves:", len(path))
            print("total_failed_moves:", 0)
            print("total_targets_reached:", int(np.sum(env.grid == 3) == 0))
